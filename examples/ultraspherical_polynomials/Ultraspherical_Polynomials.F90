...
Module Ultraspherical_Polynomials
  Use mkl_spblas
  Implicit None
...
Contains
  ...
  Subroutine Build_Backward_Transform(x)
    ...
    If (.not. Allocated(backward_transform_matrix)) Then
       ...
       backward_transform_matrix(:,1) = 0.5
       Do j=2,N
          tmp = j*acos(grid)
  ...
  Subroutine Build_Multiplication_Matrix(coefs,Ncoefs,N,lambda,matrix,stat)
    ...
    Allocate(coefs_copy(0:Ncoefs-1),data(N*N),row(N*N),column(N*N))
    ...
    row = row+1
    column = column+1
    stat = mkl_sparse_d_create_coo(tmp,SPARSE_INDEX_BASE_ONE,N,N,m,row(1:m),column(1:m),data(1:m))
    stat = mkl_sparse_convert_csr(tmp,SPARSE_OPERATION_NON_TRANSPOSE,matrix)
    stat = mkl_sparse_destroy(tmp)
    Deallocate(coefs_copy,row,column,data)
  ...
  Subroutine Build_Derivative_Matrix(N,lambda,matrix,stat)
    Implicit None
    Integer, Intent(In) :: N, lambda
    Real*8 :: data(0:N-lambda-1)
    Integer :: row(0:N-lambda-1), column(0:N-lambda-1), i
    Integer :: stat
    type(sparse_matrix_t) :: tmp
    type(sparse_matrix_t) :: matrix
    ...
    data = data + lambda
    data = 2**(lambda-1)*gamma(dble(lambda))*data

    row = row+1
    column = column+1
    stat = mkl_sparse_d_create_coo(tmp,SPARSE_INDEX_BASE_ONE,N,N,N-lambda,row,column,data)
    stat = mkl_sparse_convert_csr(tmp,SPARSE_OPERATION_NON_TRANSPOSE,matrix)
    stat = mkl_sparse_destroy(tmp)
  ...
  Subroutine Build_Conversion_Matrix(N,lambda,matrix,stat)
    Implicit None
    Integer, Intent(In) :: N, lambda
    Integer :: stat
    type(sparse_matrix_t) :: tmp
    type(sparse_matrix_t) :: matrix
    Integer :: row(2*N-2), column(2*N-2), i, j
    Real*8 :: data(2*N-2)
    ...
    Else
       ...
       Do i=2, N
          row(i) = i
          column(i) = i
          data(i) = Dble(lambda)/Dble(lambda+i-1)
       End Do
       ! Super-diagonal
       Do i=1, N-2
          row(i+N) = i
          column(i+N) = i+2
          data(i+N) = -Dble(lambda)/Dble(lambda+i+1)
       End Do
    End If
    ...
    Print*, 'Column', column
    Print*, 'Data', data
    stat =  mkl_sparse_d_create_coo(tmp,SPARSE_INDEX_BASE_ONE,N,N,2*N-2,row,column,data)
    stat = mkl_sparse_convert_csr(tmp,SPARSE_OPERATION_NON_TRANSPOSE,matrix)
    stat = mkl_sparse_destroy(tmp)
  ...
  Subroutine Build_Boundary_Matrix(N,K,conditions)
    Implicit None
    Integer, Intent(In) :: N, K, conditions(:)
    Integer :: stat
    type(sparse_matrix_t) :: tmp
    type(sparse_matrix_t) :: matrix
    Integer :: row(N*K), column(N*K), i, j, o
    Real*8 :: data(N*K), t
    ...
    Do j=1,K/2
       ...
       If (conditions(j).eq.0) Then
          Do i=0,N-1
             row((i+1)+(j-1)*N) = N-K+j
             column((i+1)+(j-1)*N) = i+1
             data((i+1)+(j-1)*N) = 1 - 2*mod(i,2)
          End Do
       ! Neumann   
       ElseIf (conditions(j).eq.1) Then
          ...
          Do i=1,N-1
             row((i+1)+(j-1)*N) = N-K+j
             column((i+1)+(j-1)*N) = i+1
             data((i+1)+(j-1)*N) = (1 - 2*mod(i,2))*i*(i+1)
          End Do
       ! Higher-order p>1   
       Else
          Do i=0,conditions(j)-1
             row((i+1)+(j-1)*N) = N-K+j
             column((i+1)+(j-1)*N) = i+1
          End Do
          ...
          Do i=conditions(j),N-1
             ...
             row((i+1)+(j-1)*N) = N-K+j
             column((i+1)+(j-1)*N) = i+1
             data((i+1)+(j-1)*N) = (1 - 2*(mod(i,2)))*t
          End Do
          data((j-1)*N+1:j*N) = data((j-1)*N+1:j*N)*(-1)**conditions(j)
       End If
    End Do
    ...
    stat = mkl_sparse_d_create_coo(tmp,SPARSE_INDEX_BASE_ONE,N,N,N*K,row,column,data)
    stat = mkl_sparse_convert_csr(tmp,SPARSE_OPERATION_NON_TRANSPOSE,matrix)
    stat = mkl_sparse_destroy(tmp)
  End Subroutine Build_Boundary_Matrix
  
End Module Ultraspherical_Polynomials
